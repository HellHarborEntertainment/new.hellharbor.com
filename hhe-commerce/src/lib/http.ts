...
export async function readJson<T>(request: Request, maxBytes = 128 * 1024): Promise<T> {
  ...
  if (!contentType.toLowerCase().includes('application/json')) throw new AppError(415, 'unsupported_media_type', 'Expected application/json');
...
export function corsHeaders(request: Request, env: Env): HeadersInit {
  ...
  return {
    ...
    'access-control-allow-headers': 'content-type,authorization,idempotency-key,x-hhe-order-token,x-turnstile-token',
...
export function errorResponse(error: unknown): Response {
  if (error instanceof AppError) {
    // Never expose provider responses, stack-adjacent data, or other upstream details to callers.
    const exposeDetails = error.status >= 400 && error.status < 500 && !['unauthorized', 'forbidden'].includes(error.code);
    return json({
      error: {
        code: error.code,
