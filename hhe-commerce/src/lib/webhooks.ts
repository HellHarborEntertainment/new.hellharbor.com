...
export async function claimWebhook(env:Env,provider:'stripe'|'spreadconnect',providerEventId:string,eventType:string,payloadSha256:string):Promise<WebhookClaim|null>{
 ...
 const row=await env.COMMERCE_DB.prepare('SELECT id,processed_at FROM webhook_events WHERE provider=? AND provider_event_id=?').bind(provider,providerEventId).first<{id:string;processed_at:string|null}>();if(!row)throw new Error('Webhook ledger row disappeared after insert');if(row.processed_at)return null;
