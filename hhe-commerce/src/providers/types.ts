...
export interface FulfillmentInput {
  orderId: string; orderNumber: string; items: CatalogItem[]; address: Address; contact: Contact;
  shippingSelection: { optionId: string; name: string; price: number; metadata?: Record<string, unknown> }; draftOrderId?: string;
}
...
export interface FulfillmentProvider {
  ...
  cancel?(env: Env, providerOrderId: string): Promise<{ status: string; raw?: unknown }>;
}
